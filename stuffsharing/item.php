...
<?php if ($is_authed): ?>
    ...
    <?php else: ?>
                ...
                <div class="row"><div class="col-xs-8"><form method="POST"><div class="input-group input-group-sm"><input class="form-control" type="number" min="<?=str_replace(array("$", ","), "", $item["pref_price"])?>" value="<?=$your_bid == false ? str_replace(array("$", ","), "", $item["pref_price"]) : str_replace(array("$", ","), "", $your_bid)?>" step="0.01" name="bid_amount" required="required"><span class="input-group-btn"><input type="hidden" name="bid" value="1" /><button type="submit" class="btn btn-success"><i class="fa fa-check" aria-hidden="true"></i> Bid</button></span></div></form><?=$to_bid ? $message : ""?></div></div>
    <?php endif ?>
<?php else: ?>
